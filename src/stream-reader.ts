...
export class StreamReader {
  ...
  constructor(stream: ReadableStream, bufferSize = ALLOCATION_SIZE) {
    this.#reader =
  ...
  async isDone() {
    // Trigger read from stream to refresh done status
    await this.#actualRead();
    return this.#done && this.#writePointer - this.#readPointer === 0;
  }
}
